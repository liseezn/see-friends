...
### v3.2.0（最新版）
#### 🎉 新增功能
1. **新增友链修改邮件验证码验证**（后台可开关，默认开启）
   - 修改申请前必须验证邮箱所有权，防止恶意提交
   - 验证码6位数字，5分钟内有效
   - 发送成功后显示60秒倒计时，防止频繁发送
   - 验证成功后自动销毁验证码，安全可靠

2. **新增新申请自动通过功能**
   - 后台可自定义开启/关闭和等待天数（默认7天）
   - 系统每天凌晨2点自动运行审核任务
   - 仅通过反链检测的申请会自动上线
   - 自动通过后同步发送邮件给申请人和管理员

#### 🚀 核心优化
1. **大幅提升反链检测准确率**，解决"实际有友链但检测不到"的90%以上问题
2. 升级真实浏览器User-Agent，绕过大部分基础反爬机制
3. 增加请求重试机制，失败后自动重试一次
4. 支持检测http/https协议差异和相对链接
5. 新增检测页面title、meta标签中的本站信息
6. 延长请求超时时间至15秒，适配慢响应站点
7. 优化请求头，支持gzip压缩，降低被拦截概率

#### 🐛 问题修复
1. **彻底修复后台设置保存不上的核心问题**
   - 改为合并旧配置，不再直接覆盖整个数组
   - 增加WordPress options缓存清理
   - 补全所有默认值，防止旧配置缺失
   - 保存后自动重新调度定时任务

2. 修复部分Cloudflare防护站点检测失败的问题
3. 修复带端口号的域名无法识别的问题
4. 修复重定向过多导致检测失败的问题
5. 修复深色模式下表单输入框文字看不清的问题

#### ⚡ 体验优化
1. 优化自动通过逻辑，增加内存清理和请求间隔
2. 完善修改表单验证，增加输入格式检查
3. 优化邮件通知内容，信息更清晰
4. 增加图标占位符配置，无图标时显示默认图片
5. 优化友链卡片布局，RSS图标位置更合理
